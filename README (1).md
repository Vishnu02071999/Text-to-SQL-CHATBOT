...
1. You type a question in the Streamlit UI (e.g. "What was the budget of Product 12?")
2. The frontend sends it to the FastAPI backend
3. The backend uses an OpenAI model (via LangChain) to convert the question into a SQL query, using your database schema as context
4. The query runs against a MySQL database and the result is sent back and displayed
...
- **Database:** hosted on [Aiven](https://aiven.io) (managed MySQL). Local MySQL instances aren't reachable from cloud-hosted backends, so the database needs to live somewhere publicly accessible.
...
- **Frontend:** deployed on [Streamlit Community Cloud](https://share.streamlit.io), pointing `BACKEND_URL` in `frontend/App.py` to the deployed Render backend URL.
